...
#undef APF_FFTW_TRAITS // This is undefined so that no more classes can be defined
    
// Alternatively, just define them seperately here
...
//  static void destroy_plan(plan p) {
//    fftwf_destroy_plan(p); p = nullptr; }
//  static void execute(const plan p) { fftwf_execute(p); }
//  static void execute_r2r(const plan p, float *in, float *out) {
//    fftwf_execute_r2r(p, in, out); }
//  static plan plan_r2r_1d(int n, float* in, float* out
//      , fftw_r2r_kind kind, unsigned flags) {
...
//    static void destroy_plan(plan p) {
//        fftw_destroy_plan(p); p = nullptr; }
//    static void execute(const plan p) { fftw_execute(p); }
//    static void execute_r2r(const plan p, double *in, double *out) {
//        fftw_execute_r2r(p, in, out); }
//    static plan plan_r2r_1d(int n, double* in, double* out
//                            , fftw_r2r_kind kind, unsigned flags) {
...
//  static void* malloc(size_t n) { return fftwl_malloc(n); }
//  static void free(void* p) { fftwl_free(p); }
//  static void destroy_plan(plan p) {
//    fftwl_destroy_plan(p); p = nullptr; }
//  static void execute(const plan p) { fftwl_execute(p); }
//  static void execute_r2r(const plan p, long double *in, long double *out) {
//    fftwl_execute_r2r(p, in, out); }
//  static plan plan_r2r_1d(int n, long double* in, long double* out
//      , fftw_r2r_kind kind, unsigned flags) {
