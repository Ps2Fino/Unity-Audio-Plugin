...
//      }
        for (int i=0; i < Pa_GetDeviceCount(); ++i) {
            result += "Device ID" + str::A2S(i) + ": "
            + Pa_GetDeviceInfo(i)->name + ", iCh: "
            + str::A2S(Pa_GetDeviceInfo(i)->maxInputChannels) + ", oCh: "
            + str::A2S(Pa_GetDeviceInfo(i)->maxOutputChannels) + ", default SR: "
            + str::A2S(Pa_GetDeviceInfo(i)->defaultSampleRate) + "\n";
        }
      return result;
    }
...
//          , _sample_rate, _block_size, 0, _pa_callback, this);
        auto err = Pa_OpenStream(&_stream, NULL, &outputParameters
                                 , _sample_rate, _block_size, 0, _pa_callback, this);
